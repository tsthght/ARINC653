function [RETURN_CODE]=SEND_BUFFER (BUFFER_ID,MESSAGE_ADDR,LENGTH,TIME_OUT)
        ...
        global Buffer_set;
  		if INVALID_ID_OF_PARTATION(BUFFER_ID)==0 
		...
		if INVALID_LENGTH_OF_PARTATION(LENGTH, BUFFER_ATTRIBUTE.MAX_MESSAGE_SIZE)==1
		    
			RETURN_CODE = RETURN_CODE_TYPE.INVALID_PARAM;
			return;
        end
        
		if INVALID_MESSAGE_ADDR_OF_PARTATION(MESSAGE_ADDR) == 1
		    
			RETURN_CODE = RETURN_CODE_TYPE.INVALID_PARAM;
			return;
        end
        
		if INVALID_TIME_OUT_OF_PARTATION(TIME_OUT) 
		    
			RETURN_CODE = RETURN_CODE_TYPE.INVALID_PARAM;
			return;
        end
  		
        index=FIND_BUFFER_INDEX_OF_PARTATION( BUFFER_ID );
...
%                 WBPro_ID=Waiting_Buffer_Set(1);
                ...
                COPY_OF_PARTATION(BUFFER_ID,MESSAGE_ADDR,LENGTH);
                 Buffer_set{1,index}.NB_MESSAGE = Buffer_set{1,index}.NB_MESSAGE + 1;
...
%                 Waiting_Processes_set{1,length(Waiting_Processes_set)+1} = PRO.ID;
                Locate = find_OF_PARTATION(PRO.ID);
